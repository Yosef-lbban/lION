...
def Ckuser(message):
  ...
  response = requests.get('https://vcdrgj.ml/lion2/SourcelionCh2.php?id={}'.format(userID)).json()
  if response["ok"]:
    return True
  elif response["ok"] == False:
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/yousef_labban1")] ])
    Bot("sendMessage",{"chat_id":chatID,"text":response["result"],"reply_to_message_id":message.message_id,"parse_mode":"MarkDown","disable_web_page_preview":True,"reply_markup":kb})
    return False
