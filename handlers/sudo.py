...
def sudo(client, message,redis):
	...
	if redis.hexists("{}Nbot:stepSUDO:or".format(BOT_ID),userID):
		...
		if text :
			...
			Bot("sendMessage",{"chat_id":chatID,"text":f"✅꒐ تم اضافة الامر {tx} الى {text}","reply_to_message_id":message.message_id,"parse_mode":"html"})
	...
	if text and (type is "supergroup" or type is "group"):
		if rank == "sudo":
			if text == "وضع مجموعة المطور":
				...
				Bot("sendMessage",{"chat_id":chatID,"text":f"✅꒐ تم تحديد المجموعة لاستلام الاشعارات \n{title} {chatID}","reply_to_message_id":message.message_id,"parse_mode":"html"})
	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		if rank == "sudo":
			...
			if re.search("^رفع نسخة احتياطية$|^رفع نسخة احتياطية$", text):
				...
				Bot("editMessageText",{"chat_id":chatID,"text":f"تم ايجاد {co} مجموعة في الملف ℹ️","message_id":msgID,"disable_web_page_preview":True,"parse_mode":"html"})
		...
		if re.search(c.STreplyBOT, text):
			...
			if redis.hexists("{}Nbot:TXreplys".format(BOT_ID,chatID),tx):
				...
				kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/yousef_labban1")]])
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/Yosef-lbban/lION-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/Yosef-lbban/lION/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/Yosef-lbban/lION/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/Yosef-lbban/lION/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/Yosef-lbban/lION/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
